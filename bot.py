...
# Sistema de configuración de paneles de tickets
ticket_panel_config_file = 'ticket_panel_config.json'
if os.path.exists(ticket_panel_config_file):
  with open(ticket_panel_config_file, 'r') as f:
      ticket_panel_configs = json.load(f)
else:
  ticket_panel_configs = {}
...
def save_ticket_panel_configs():
  with open(ticket_panel_config_file, 'w') as f:
      json.dump(ticket_panel_configs, f, indent=4)


def get_panel_config(guild_id):
  """Obtener configuración del panel de tickets para un servidor"""
  guild_id = str(guild_id)
  if guild_id not in ticket_panel_configs:
      ticket_panel_configs[guild_id] = {
  ...
  return ticket_panel_configs[guild_id]


def set_panel_config(guild_id, title=None, description=None, footer=None):
...
class TicketView(discord.ui.View):
  ...
  def setup_category_select(self):
      ...
      for category_id, category_data in categories.items():
          if len(options) >= 25:  # Discord límite de 25 opciones
              break

          # Determinar emoji basado en el color
          emoji_map = {
              'red': '🔴',
              'green': '🟢',
              'blue': '🔵',
...
async def setup_tickets(interaction: discord.Interaction):
  ...
  # Obtener configuración personalizada del panel
  panel_config = get_panel_config(interaction.guild.id)
...
class TicketCategoryMenuView(discord.ui.View):
  ...
  @discord.ui.select(
      ...
      options=[
          ...
          discord.SelectOption(label="🎨 Editar Texto del Panel",
                               description="Personalizar título, descripción y footer",
                               emoji="🎨",
                               value="edit_panel_text"),
          discord.SelectOption(label="👁️ Ver Configuración del Panel",
                               description="Ver texto actual del panel",
                               emoji="👁️",
                               value="view_panel_config"),
  ...
  async def select_ticket_action(self, interaction: discord.Interaction,
      ...
      elif selected == "edit_panel_text":
          embed = discord.Embed(
              title="🎨 Editar Texto del Panel",
              description="Para personalizar el texto del panel de tickets, usa:\n\n"
              "**Comandos:**\n"
              "`/tpanel_title <nuevo_título>` - Cambiar título del panel\n"
              "`/tpanel_desc <nueva_descripción>` - Cambiar descripción del panel\n"
              "`/tpanel_footer <nuevo_footer>` - Cambiar texto del footer\n"
              "`/tpanel_reset` - Restaurar texto por defecto\n\n"
              "**Ejemplos:**\n"
              "`/tpanel_title \"🎫 Centro de Soporte 24/7\"`\n"
              "`/tpanel_desc \"¡Bienvenido! Selecciona una opción para obtener ayuda.\"`",
              color=discord.Color.purple())
      elif selected == "view_panel_config":
          await self.show_panel_config(interaction)
          return
  ...
  async def show_panel_config(self, interaction):
      guild_id = str(interaction.guild.id)
      panel_config = get_panel_config(guild_id)

      embed = discord.Embed(
          title="👁️ Configuración Actual del Panel",
          description="Texto actual configurado para el panel de tickets:",
      ...
      embed.add_field(name="⚙️ Comandos de Edición",
                      value="• `/tpanel_title` - Cambiar título\n"
                            "• `/tpanel_desc` - Cambiar descripción\n"
                            "• `/tpanel_footer` - Cambiar footer\n"
                            "• `/tpanel_reset` - Restaurar por defecto",
                      inline=False)
...
class TicketAdminMenuView(discord.ui.View):
  ...
  async def setup_panel(self, interaction):
      ...
      confirm_embed = discord.Embed(
          ...
          f"• Texto personalizable configurado",
          color=discord.Color.green())
...
@discord.app_commands.describe(title="Nuevo título para el panel de tickets")
async def ticket_panel_title(interaction: discord.Interaction, title: str):
  if not interaction.user.guild_permissions.manage_channels:
      await interaction.response.send_message(
          "❌ Necesitas permisos de **Administrar Canales**.", ephemeral=True)
      return
  ...
  if len(title) > 256:
      await interaction.response.send_message(
          "❌ El título no puede tener más de 256 caracteres.", ephemeral=True)
      return

  guild_id = interaction.guild.id
  set_panel_config(guild_id, title=title)
  ...
  try:
      ...
      print(f"Error actualizando paneles tras cambiar título: {e}")


@bot.tree.command(name="tpanel_desc", description="Cambiar descripción del panel de tickets")
@discord.app_commands.describe(description="Nueva descripción para el panel de tickets")
async def ticket_panel_description(interaction: discord.Interaction, description: str):
  if not interaction.user.guild_permissions.manage_channels:
      await interaction.response.send_message(
          "❌ Necesitas permisos de **Administrar Canales**.", ephemeral=True)
      return
  ...
  if len(description) > 4000:
      await interaction.response.send_message(
          "❌ La descripción no puede tener más de 4000 caracteres.", ephemeral=True)
      return

  guild_id = interaction.guild.id
  set_panel_config(guild_id, description=description)
  ...
  try:
      ...
      print(f"Error actualizando paneles tras cambiar descripción: {e}")


@bot.tree.command(name="tpanel_footer", description="Cambiar footer del panel de tickets")
@discord.app_commands.describe(footer="Nuevo footer para el panel de tickets")
async def ticket_panel_footer(interaction: discord.Interaction, footer: str):
  if not interaction.user.guild_permissions.manage_channels:
      await interaction.response.send_message(
          "❌ Necesitas permisos de **Administrar Canales**.", ephemeral=True)
      return
  ...
  if len(footer) > 2048:
      await interaction.response.send_message(
          "❌ El footer no puede tener más de 2048 caracteres.", ephemeral=True)
      return

  guild_id = interaction.guild.id
  set_panel_config(guild_id, footer=footer)
  ...
  try:
      ...
      print(f"Error actualizando paneles tras cambiar footer: {e}")


@bot.tree.command(name="tpanel_reset", description="Restaurar texto por defecto del panel de tickets")
async def ticket_panel_reset(interaction: discord.Interaction):
  if not interaction.user.guild_permissions.manage_channels:
      await interaction.response.send_message(
          "❌ Necesitas permisos de **Administrar Canales**.", ephemeral=True)
      return
  ...
  try:
      ...
      print(f"Error actualizando paneles tras reset: {e}")


@bot.tree.command(name="tpanel_view", description="Ver configuración actual del panel de tickets")
async def ticket_panel_view(interaction: discord.Interaction):
  if not interaction.user.guild_permissions.manage_channels:
      await interaction.response.send_message(
          "❌ Necesitas permisos de **Administrar Canales**.", ephemeral=True)
      return
  ...
  embed = discord.Embed(
      title="👁️ Configuración Actual del Panel",
      description="Texto configurado para el panel de tickets en este servidor:",
      color=discord.Color.cyan())
  ...
  embed.add_field(name="⚙️ Comandos de Edición",
                  value="• `/tpanel_title <título>` - Cambiar título\n"
                        "• `/tpanel_desc <descripción>` - Cambiar descripción\n"
                        "• `/tpanel_footer <footer>` - Cambiar footer\n"
                        "• `/tpanel_reset` - Restaurar por defecto",
                  inline=False)
